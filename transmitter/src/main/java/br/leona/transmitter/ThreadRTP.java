		...
		while (!isCancelled()) {
			...
			if (connect) {
				...
				transmissor = new Transmitter(new MediaLocator("vfw://0"),
						hostAddress, 1235, new Format(VideoFormat.JPEG));  //1235 � a port de transmissao RTP, tem que ser diferente da TCP
																		 //aqui estamos especificando por qual port sera o envio das informa��es de video
				String result = transmissor.start();
				if (result != null) {
					System.err.println("Error : " + result);
				}
				System.err
						.println("Transmissão RTP iniciada para " + hostAddress);
			}
		}
		if (transmissor != null) {
			transmissor.stop();
