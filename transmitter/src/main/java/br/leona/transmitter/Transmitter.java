...
public class Transmitter {
	...
	public Transmitter(MediaLocator locator, String hostAddress, int port,
			Format format) {
		...
		synchronized (this) {
			if (processor != null) {
				...
				for (int i = 0; i < messageRTP.length; i++) {
					messageRTP[i].removeTargets("Sess�o terminada.");
	...
	private String createProcessor() {
		...
		try {
			sataSource = javax.media.Manager.createDataSource(locator);
		} catch (Exception e) {
			return "Couldn't create DataSource";
		}
		try {
			processor = javax.media.Manager.createProcessor(sataSource);
		...
		for (int i = 0; i < pbss.length; i++) {
			try {
				...
				System.err.println("Created RTP session: " + hostAddress + " "
						+ port);
				sendStream = messageRTP[i].createSendStream(dataOutput, i);
				sendStream.start();
			} catch (Exception e) {
				return e.getMessage();
			}
